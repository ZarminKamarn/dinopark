...
export const paymentSchema = z.object({
  cardNumber: z
    .string()
    .refine(cardNumberChecker, "Le numéro de carte est invalide"),
  CVV: z
    .string()
    .min(3, "Le CVV fait 3 caractères minimum")
    .max(4, "Le CVV fait 4 caractères maximum")
    .refine(CVVChecker, "Seuls les chiffres sont autorisés"),
  expiryDate: z
    .string()
    .length(7, "La date d'expiration est invalide")
    .refine(expiryDateFormatChecker, "La date d'expiration est invalide")
    .refine(expiryDateChecker, "La date d'expiration est passée"),
...
export const bookingSchema = z.object({
  park: z.coerce
    .number("Merci de sélectionner un parc")
    .min(1, "Le parc sélectionné est invalide"),
  ticket: z
    .array(z.string())
    .refine(ticketsChecker, "Seuls les nombres positifs ou nuls sont acceptés"),
  bookingDate: z.coerce
    .date("Date de réservation invalide")
    .min(
      new Date(),
      "La réservation doit avoir une date ultérieur à celle du jour"
    ),
  lastName: z
    .string()
