...
export class AttractionController extends Controller {
    public async attractions(){
        ...
        const attractions: Array<Attraction> = await attractionRepository.findAll<Attraction>("attraction");
    ...
    public async attraction(){
        const attractionRepository = new AttractionRepository();
        const attraction: Attraction | null = await attractionRepository.findById<Attraction>("attraction", this.request.params.id);

        if(attraction){
            const dinosaurRepository = new DinosaurRepository();
            const dinosaurs: Array<Dinosaur> = await dinosaurRepository.findByAttraction(this.request.params.id);
