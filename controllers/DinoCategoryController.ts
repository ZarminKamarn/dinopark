...
export class DinoCategoryController extends Controller {
  public async categories() {
    ...
    const categories: Array<DinosaurCategory> =
      await categoryRepository.findAll<DinosaurCategory>("dinosaurcategory");
  ...
  public async category() {
    ...
    if (category) {
      const dinosaurRepository = new DinosaurRepository();
      const dinosaurs: Array<Dinosaur> =
        await dinosaurRepository.findByCategory(this.request.params.id);
